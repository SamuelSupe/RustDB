...
use crate::{
    ...
    sql::BoundExpr,
    storage::{NativeDeleteSegment, NativeDeleteVector, NativeDeleteWriter, PreparedSnapshot},
};
...
pub(super) fn apply(
    ...
    predicate: &BoundExpr,
    matches: Option<&super::super::native_matches::DeleteMatches>,
    returning: Option<&ReturningProjection>,
    memory: &MemoryPool,
    batch_size: usize,
    control: &QueryControl,
    mut writer: NativeDeleteWriter,
    ...
    match result {
        Ok(()) => writer.finish().map(|prepared| (prepared, returned)),
        Err(error) => match writer.abort() {
            Ok(()) => Err(error),
            Err(cleanup) => Err(Error::native_storage(
                root,
                format!("{error}; native DELETE cleanup failed: {cleanup}"),
            )),
        },
    }
}
...
fn delete_segment(
    ...
    for batch in &mut reader {
        ...
        let mask = match matches {
            Some(matches) => matches.mask(&batch, &vector, offset)?,
            None => visible_delete_mask(predicate, &batch, &vector, offset)?,
        };
        if mask.true_count() != 0 {
            if let Some(returning) = returning {
                ...
                returned.push(BatchEnvelope::try_new(batch, memory, "DELETE RETURNING")?);
            }
    ...
    if vector.deleted_rows() != previous {
        writer.write_vector(segment.segment_id(), &vector)?;
    }
    Ok(())
}

fn visible_delete_mask(
    predicate: &BoundExpr,
    ...
    let selected = selected
        ...
        .ok_or_else(|| Error::Internal("DELETE predicate did not produce Boolean".to_owned()))?;
